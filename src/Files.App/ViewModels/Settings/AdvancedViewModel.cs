...
namespace Files.App.ViewModels.Settings
{
	public sealed partial class AdvancedViewModel : ObservableObject
	{
		...
		private async Task SetAsDefaultExplorerAsync()
		{
			...
			{
				if (!await Win32Helper.RunPowershellCommandAsync($"-command \"New-Item -Force -Path '{dataPath}' -ItemType Directory; Copy-Item -Filter *.* -Path '{destFolder}\\*' -Recurse -Force -Destination '{dataPath}'; 'lyra' | Out-File -Encoding utf8 -Force -FilePath '{dataPath}\\Branch.txt'\"", PowerShellExecutionOptions.Hidden))
				{
					// Error copying files
					await DetectResult();
					return;
